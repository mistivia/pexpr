...
/* Construction. All return NULL on allocation failure. */
...
/* Recursively frees a node and everything it owns. NULL-safe. */
 ...
 * must be freed separately with pnode_free(). NULL-safe (returns NULL
 * for a NULL `node`). Returns NULL on allocation failure. */
 ...
 * syntax error, empty input, or allocation failure. Trailing bytes after
 * the value (if any) are ignored. */
 ...
 * owns it, free with pnode_free()) and resets `self` so it is ready to
