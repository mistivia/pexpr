...
static void test_misuse(void) {
    ...
    CHECK_EQ_LL(pnode_list_append(&scalar, pnode_new_integ(2)), -1);
    CHECK_EQ_LL(pnode_list_append(NULL, pnode_new_integ(2)), -1);

    CHECK_EQ_LL(pnode_list_len(&scalar), 0);
    CHECK_EQ_LL(pnode_list_len(NULL), 0);
    pnode_free(&scalar);

    /* pnode_free must be NULL-safe, and safe to call twice in a row. */
    pnode_free(NULL);
    struct pnode n = pnode_new_str("x", 1);
    pnode_free(&n);
    pnode_free(&n);
}
